...
// Sql
const WhatsAsenaDB = config.DATABASE.define('DUMI BOT', {
    info: {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('DUMI BOT😼🤝' + afwhasena));
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\nDUMI BOT😼🤝\n\n  Working as Private! \n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using DUMI BOT 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [DUMI BOT😼🤝] --*' + 
                                    '\n*White Dragon bir hata gerçekleşti!*'+
                                    '\n_Bu hata logunda numaranız veya karşı bir tarafın numarası olabilir. Lütfen buna dikkat edin!_' +
                                    '\n_Yardım için Telegram grubumuza yazabilirsiniz._' +
                                    '\n_Bu mesaj sizin numaranıza (kaydedilen mesajlar) gitmiş olmalıdır._' +
                                    '\n_Hatayı https://chat.whatsapp.com/La5MMeWqcLK0Msq50LvnqX bu gruba iletebilirsiniz._\n\n' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [DUMI BOT😼🤝] --*' + 
                                    ...
                                    '\n*Aslo you can join our support group:* https://chat.whatsapp.com/HmalrdeVMtjDR2MUD1AnBf' +
                                    '\n*This message should have gone to your number (saved messages).*\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [DUMI BOT😼🤝] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
